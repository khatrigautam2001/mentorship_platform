...
export async function registerRoutes(app: Express): Promise<Server> {
        ...
        const studentsWithProgress = await Promise.all(
          mentees.map(async (mentee) => {
            ...
            if (hasIndividualCustomization) {
              ...
              skillsWithItems = await Promise.all(
                allSkills.map(async (skill) => {
                  const items = await storage.getRoadmapItemsBySkillId(
                    skill.id,
                  );
                  return { ...skill, items };
                }),
  ...
  app.get(
    "/api/mentor/students/:id/credentials",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params;
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        const mentees = await storage.getMenteesByMentorId(req.session.userId!);
        const requests = await storage.getMockInterviewRequestsByMentorId(
          req.session.userId!,
        );
        ...
        for (const mentee of mentees) {
          const paymentsRecords = await storage.getPaymentsByMenteeId(
            mentee.id,
          );
          const totalPaid = paymentsRecords.reduce(
            (sum, p) => sum + parseFloat(p.amount.toString()),
            0,
          );
          const totalFee = mentee.totalFee
            ? parseFloat(mentee.totalFee.toString())
            : 0;
        ...
        for (const mentee of mentees) {
          ...
          const percentage =
            allRoadmapItems.length > 0
              ? (completedItems / allRoadmapItems.length) * 100
              : 0;
          totalProgress += percentage;
        }

        const avgCompletion =
          mentees.length > 0 ? Math.round(totalProgress / mentees.length) : 0;
  ...
  app.post(
    "/api/mentor/mock-requests/:id/approve",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params;
        ...
        if (mockItem) {
          ...
          const mockProgress = existingProgress.find(
            (p) => p.itemId === mockItem.id,
          );
          ...
          const allSkills = await storage.getAllSkills();
          const currentSkillIndex = allSkills.findIndex(
          ...
          if (
            ...
            const nextSkillItems = await storage.getRoadmapItemsBySkillId(
              nextSkill.id,
            );
  ...
  app.post(
    "/api/mentor/mock-requests/:id/reject",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params;
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        // Check if mentee has individual roadmap customization
        const individualItems =
          await storage.getIndividualRoadmapItemsByMenteeId(
            req.session.userId!,
          );
          ...
          const individualSkillsWithItems = await Promise.all(
            individualSkills.map(async (skill) => {
              const items = await storage
                .getIndividualRoadmapItemsByMenteeId(req.session.userId!)
                .then((allItems) =>
                  allItems.filter(
                    (item) => item.individualSkillId === skill.id,
                  ),
  ...
  app.post(
    "/api/mentee/complete-item/:itemId",
    requireMentee,
    async (req: Request, res: Response) => {
      try {
        ...
        for (let i = 0; i < currentIndex; i++) {
          ...
          const previousProgress = existingProgress.find(
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        const badgesList = await storage.getBadgesByMenteeId(
          req.session.userId!,
        );
        const allSkills = await storage.getAllSkills();
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const totalFee = user.totalFee
          ? parseFloat(user.totalFee.toString())
          : 0;
  ...
  app.get(
    "/api/roadmap/individual/:menteeId/skills",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        ...
        if (!mentee || mentee.mentorId !== req.session.userId) {
          return res
            .status(403)
            .json({ message: "Forbidden - Mentee does not belong to you" });
        }
  ...
  app.get(
    "/api/roadmap/individual/:menteeId",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        const { menteeId } = req.params;
        ...
        const individualItems =
          await storage.getIndividualRoadmapItemsByMenteeId(menteeId);
        const individualSkills =
          await storage.getIndividualSkillsByMenteeId(menteeId);
        ...
        if (individualItems.length > 0 || individualSkills.length > 0) {
          ...
          const individualSkillsWithItems = await Promise.all(
            individualSkills.map(async (skill) => {
              const items = await storage
                .getIndividualRoadmapItemsByMenteeId(menteeId)
                .then((allItems) =>
                  allItems.filter(
                    (item) => item.individualSkillId === skill.id,
                  ),
          ...
          const allSkillsWithItems = [
            ...skillsWithItems,
            ...individualSkillsWithItems,
          ];
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const skill = await storage.createIndividualSkill(
          menteeId,
          name,
          description,
        );
  ...
  app.patch(
    "/api/roadmap/individual/:menteeId/skills/:skillId",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        ...
        const skill = await storage.updateIndividualSkill(skillId, {
          name,
          description,
        });
        res.json(skill);
  ...
  app.post(
    "/api/roadmap/individual/:menteeId/reset",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        const { menteeId } = req.params;
  ...
  app.post(
    "/api/roadmap/individual/:menteeId/items",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        const { menteeId } = req.params;
  ...
  app.patch(
    "/api/roadmap/individual/:menteeId/items/:itemId",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
  ...
  app.patch(
    "/api/roadmap/individual/:menteeId/items/:itemId/reorder",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        const { menteeId, itemId } = req.params;
  ...
  app.patch(
    "/api/roadmap/individual/:menteeId/skills/:skillId/reorder",
    requireMentor,
    async (req: Request, res: Response) => {
      try {
        const { menteeId, skillId } = req.params;
